...
public:
    ...
    Result<void> open(const std::string& path) noexcept {
        ...
        LOG_INFO("WAL opened: %s  durability=%s  flush_ms=%d",
                 path.c_str(),
                 cfg_.durability == DurabilityMode::Unsafe  ? "unsafe" :
                 cfg_.durability == DurabilityMode::Durable ? "durable" : "strict",
    ...
    Result<void> replay(std::function<void(const LogEntry&)> cb) noexcept {
        ...
        for (;;) {
            ...
            if (n == 0) break;
            if (n != 12) { LOG_WARN("WAL: truncated header at %llu", (unsigned long long)records); break; }
            ...
            if ((uint32_t)read(fd_, data.data(), data_len) != data_len) {
                LOG_WARN("WAL: truncated data at %llu", (unsigned long long)records); break;
            }
            ...
            LogEntry le;
            le.term=r.u64(); le.index=r.u64();
            le.op=static_cast<Cmd>(r.u8());
            le.key=r.str(); le.val=r.str();
            cb(le);
            ++records;
        }
...
private:
    ...
    void flush_loop() {
        while (flush_running_.load(std::memory_order_relaxed)) {
            std::vector<PendingEntry*> batch;
            {
                ...
                pending_cv_.wait_for(lk,
                    ...
                    [this]{ return !pending_.empty() || !flush_running_.load(); });
        ...
        { std::lock_guard lk(pending_mtx_); remainder = std::move(pending_); }
        if (!remainder.empty()) flush_batch(remainder);
    }
    ...
    void flush_batch(std::vector<PendingEntry*>& batch) {
        ...
        bool ok = true;
        {
            std::lock_guard lk(mtx_);
            ssize_t total = 0;
            for (auto& v : iovs) total += (ssize_t)v.iov_len;
    ...
    static std::vector<uint8_t> serialize(const LogEntry& e) {
        ...
        memcpy(hdr+0,&m,4); memcpy(hdr+4,&l,4); memcpy(hdr+8,&c,4);
